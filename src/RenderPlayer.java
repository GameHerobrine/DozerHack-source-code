...
public class RenderPlayer extends RenderLiving {
            ...
            if (var10 < var11) {
                ...
                if (!var1.isSneaking()) {
                    if (var1.isPlayerSleeping()) {
                    	...
                    	int distance = (int) var10;
                    	String var404 = new StringBuilder().append("\2476(").append(distance).append(")").toString();
                        this.renderLivingLabel(var1, var12 + var404, var2, var4, var6, 64);
                    }
                } else {
                    ...
                    var13.drawString(var12, -var13.getStringWidth(var12) / 2, 0, 0xffffffff);
    ...
    public static void drawPlayerESP(double d, double d1, double d2) {
		...
		GL11.glDepthMask(false);
		GL11.glEnable(GL11.GL_LINE_SMOOTH);
		RenderGlobal.drawOutlinedBoundingBox(new AxisAlignedBB(d - 0.5D, d1 + -0.01, d2 - 0.5D, d + 0.5D, d1 + 2D, d2 + 0.5D));
		GL11.glColor4f(255.0F, 0.0F, 0.0F, 0.123456789F);
		RenderGlobal.drawBoundingBox(new AxisAlignedBB(d - 0.5D, d1 + -0.01, d2 - 0.5D, d + 0.5D, d1 + 2D, d2 + 0.5D));
		GL11.glDepthMask(true);
		GL11.glEnable(3553 /* GL_TEXTURE_2D */);
